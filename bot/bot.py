...
⚪ /retry – Перегенерировать ответ на предыдущий запрос
...
🎨 Создавайте изображения на основе текстовых подсказок в <b>👩‍🎨 Дизайнер</b> /mode
👥 Добавляйте бота в <b>групповые чаты</b>: /help_group_chat
🎤 Отправляйте <b>голосовые сообщения</b> вместо текста
"""
...
3. Вы молодец!
Чтобы получить ответ от бота в чате - упомяните его с помощью <b>tag</b> или <b>ответьте</b> на его сообщение.
...
async def start_handle(update: Update, context: CallbackContext):
    ...
    reply_text = "Привет! Я бот <b>ChatGPT</b>, реализованный с помощью OpenAI API 🤖\n\n"
...
async def message_handle(update: Update, context: CallbackContext, message=None, use_new_dialog_timeout=True):
    ...
    async def message_handle_fn():
        ...
        try:
            ...
            error_text = f"Что-то пошло не так во время выполнения. Причина: {e}"
...
async def is_previous_message_not_answered_yet(update: Update, context: CallbackContext):
    ...
    if user_semaphores[user_id].locked():
        ...
        text += "Или вы можете отменить это с помощью команды /cancel."
...
async def generate_image_handle(update: Update, context: CallbackContext, message=None):
    ...
    try:
        ...
        if str(e).startswith("Your request was rejected as a result of our safety system"):
            text = "🥲 Ваш запрос <b>не соответствует</b> политике использования OpenAI. Что вы написали там, а?"
            await update.message.reply_text(text, parse_mode=ParseMode.HTML)
            return
        else:
            raise
...
def get_chat_mode_menu(page_index: int):
    ...
    text = f"Выберите <b>режим общения</b> ({len(config.chat_modes)} режимов доступно):"
